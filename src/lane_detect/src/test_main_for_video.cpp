...
int main(int argc, char **argv){
    ...
    start = true;
    // cap.open("../../../src/lane_detect/source/test_video_1.mp4");
    // cap.set(CAP_PROP_POS_FRAMES, 57300);
    // cap.open("../../../src/lane_detect/source/vid1.avi");
    navigationSubscriber.shutdown();
    //

    string source = "challenge.mp4";
    ...
    cap.open("../../../src/lane_detect/source/" + source);
    ...
    VideoWriter wri;
    wri.open("../../../src/lane_detect/source/test_images/"+source+"_demo.mp4", VideoWriter::fourcc('M', 'J', 'P', 'G'), 30 , Size(1280, 720), true);
    while(ros::ok()){
    // for(int i=0; i<500; i++) {
        ...
        if(key=='q') break;
            ...
            // cv::imwrite("../../../src/lane_detect/source/test_images/source.jpg", src);
            // cv::imwrite("../../../src/lane_detect/source/test_images/top_view.jpg", transfomredView);
            pauseFlag = !pauseFlag;
        ...
        Mat src;// = imread("../../../src/lane_detect/source/test_images/source.jpg");
        cap >> src;
        if(src.empty()) {
        ...
        // cv::imwrite("../../../src/lane_detect/source/test_images/top_hat.jpg", filtered);

        Mat colorFilter = LaneDetector::getColorFilter(filtered);

        imshow("colorFilter", colorFilter);
        // cv::imwrite("../../../src/lane_detect/source/test_images/color_filter.jpg", colorFilter);
        ...
        erode(lane, lane, Mat(Size(3, 5), CV_8U, {1}));
        dilate(lane, lane, Mat(Size(3, 5), CV_8U, {1}));
        // erode(lane, lane, Mat(Size(7, 7), CV_8U, {1}));
        // dilate(lane, lane, Mat(Size(7, 7), CV_8U, {1}));
        
        imshow("lane", lane);
        // cv::imwrite("../../../src/lane_detect/source/test_images/lane.jpg", lane);
        ...
        imshow("lanebox", lane);
        // cv::imwrite("../../../src/lane_detect/source/test_images/lane_box.jpg", lane);
        ...
        cv::Mat drawLine;
        cvtColor(lane, drawLine, COLOR_GRAY2BGR);
        // transfomredView.copyTo(drawLine);
        LaneDetector::drawLine(drawLine, fittedLeft, fittedRight, Scalar(255, 0, 255), 1);
        // cv::imwrite("../../../src/lane_detect/source/test_images/lane_line.jpg", drawLine);
        imshow("tr", drawLine);


        Mat res;
        resize(src, res, Size(1280, 720));
        // vector<Point2d> invPTCPoints = LaneDetector::getInversePerspectiveTransformedPoints(fittedCenter, pMat, 2);
        // LaneDetector::drawLine(res, invPTCPoints, Scalar(255, 0, 255), 2);

        vector<Point2d> invPTLPoints = LaneDetector::getInversePerspectiveTransformedPoints(fittedLeft, pMat, 2);
        vector<Point2d> invPTRPoints = LaneDetector::getInversePerspectiveTransformedPoints(fittedRight, pMat, 2);
        LaneDetector::drawLineArea(res, invPTLPoints, invPTRPoints, Scalar(255, 0, 255));
        // cv::imwrite("../../../src/lane_detect/source/test_images/area.jpg", res);
        imshow("res", res);

        resize(src, src, Size(1280/3, 720/3));
        resize(transfomredView, transfomredView, Size(1280/3, 720/3));
        resize(drawLine, drawLine, Size(1280/3, 720/3));
        src.copyTo(res(Rect(0, 0, 1280/3, 720/3)));
        transfomredView.copyTo(res(Rect(1280/3+1, 0, 1280/3, 720/3)));
        drawLine.copyTo(res(Rect(1280/3*2+1, 0, 1280/3, 720/3)));
        imshow("dst", res);
        wri<<res;
